...
io.on('connection', function(socket){
	...
	socket.on('submitPlayerData', function(data){
	   ...
	   if (!io.sockets.adapter.rooms.has(roomData.roomName)) {
			socket.join(roomData.roomName);
			console.log(io.sockets.adapter.rooms.get(roomData.roomName).size);
			if (board.pOne.playerName.length < 1) {
				board.pOne.playerID = socket.id;
				board.pOne.rn = roomData.roomName;
		...
		else if (io.sockets.adapter.rooms.has(roomData.roomName)) {
			console.log(io.sockets.adapter.rooms.get(roomData.roomName).size);
			if (io.sockets.adapter.rooms.get(roomData.roomName).size < 2 /*roomData.noOfPlayers < 2*/) {
				if (board.pOne.playerName.length < 1) {
					board.pOne.playerID = socket.id;
					board.pOne.rn = roomData.roomName;
					board.pOne.playerName = data.pn;
					...
					board.pTwo.rn = roomData.roomName;
